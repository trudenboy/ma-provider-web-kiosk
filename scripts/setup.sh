...
# Dev environment setup for the web_kiosk provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/web_kiosk"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: web_kiosk → $TARGET"
  ...
  echo "Provider web_kiosk linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
  ...
  PROVIDER_TARGET="$REPO_ROOT/ma-server/music_assistant/providers/web_kiosk"
